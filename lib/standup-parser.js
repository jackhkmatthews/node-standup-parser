...
class StandupParser {
  ...
  parse(folderPath) {
    const filepath = this.getFilePath(folderPath);
  ...
  getFilePath(folderPath) {
    const inputFileName = this.getFileName(folderPath);
    return `${folderPath}/${inputFileName}.txt`;
  }
  ...
  getDate(string) {
    return moment.utc(string.match(/..\/..\/..../ig)[0], 'DD-MM-YYYY').toDate();
  }
  ...
  getDayObjectLog(dayObject) {
    if (dayObject.positions.length !== dayObject.summaries.length) {
      return ['error', `${dayObject.date.toDateString()} has incomplete data and has not been parsed`];
    }
    return ['success', `${dayObject.date.toDateString()} has complete data`];
  }
